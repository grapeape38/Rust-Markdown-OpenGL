...
fn add_random_shapes(draw_list: &mut DrawList, vp: &Point, n: u8) {
    ...
    let rand_shape = || {
        ...
        if r == 3 {
            let p2 = rand_pt();
            LineBuilder::new()
                .points(p1.0 as f32, p1.1 as f32, p2.0 as f32, p2.1 as f32)
...
fn test_draw() {
    ...
    let text_params = TextParams::new(test_str)
        .offset(&Point::new(300., 300.))
        .color(255, 0, 255)
        .rot(Degrees(0.).into());
    'main: loop {
            ...
            match event {
                ev @ Event::MouseMotion { .. } => {
                    if timer.elapsed().unwrap() >= Duration::from_millis(5) {
                        //don't always handle mouse move
                        app_state.handle_mouse_event(&ev, &kmod);
                        timer = SystemTime::now();
                    }
                }
                ev @ Event::MouseButtonDown { .. } | ev @ Event::MouseButtonUp { .. } => {
                    //always handle mouse down and up
                    app_state.handle_mouse_event(&ev, &kmod);
                }
                Event::Quit { .. }
                | Event::KeyDown {

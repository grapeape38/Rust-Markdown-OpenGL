...
use crate::primitives::{rgb_to_f32, DrawCtx, Point, Radians, Rect, RotateRect, TransformCache};
...
impl TextParams {
    ...
    pub fn get_uniforms(
        ...
        /*match self.align {
            TextAlign::Left => {}
            TextAlign::Center => {
                let width = rt.measure(line, self.scale).x;
...
impl RenderText {
    pub fn new() -> Result<Self, Box<dyn Error>> {
        ...
        let face = lib
            .new_face("fonts/arial.ttf", 0)
            .map_err(|e| format!("Could not load font face: {:?}", e))?;
        let mut char_map = HashMap::new();
        face.set_char_size(12 * 64, 0, 50, 0).unwrap();
        for c in 0..=127 {
            face.load_char(c as usize, freetype::face::LoadFlag::RENDER)
                .map_err(|e| format!("Could not load char {:?} {:?}", c, e))?;
    ...
    fn char_offset(&self, scale: f32) -> Point {
        ...
        Point::new(ch.bearing.x as f32 * scale, ch.size.y as f32 * scale)
    }
    ...
    pub fn char_size(&self, ch: char, scale: f32) -> Point {
        self.char_map
            .get(&(ch as GLchar))
            .map(|ch| Point::new(scale * ch.size.x as f32, scale * ch.size.y as f32))
            .unwrap_or(Point::origin())
    }
    pub fn char_size_w_advance(&self, ch: char, scale: f32) -> Point {
        self.char_map
            .get(&(ch as GLchar))
            .map(|ch| Point::new(scale * (ch.advance >> 6) as f32, scale * ch.size.y as f32))
            .unwrap_or(Point::origin())
    }
